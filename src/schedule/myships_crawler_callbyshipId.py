...
# 206 是 2
serial_list = [0, 1, 2]
if not os.environ.get('SERIAL'):
    os.environ['SERIAL'] = f"{serial_list[0]}"
print(os.environ['SERIAL'])
...
class myships_crawler_class():
    ...
    def get_ship_detail(self, shipId_list_for_func):
        ...
        try:
            rsp = RequestsRetryer('post', {'url':app.config['CRAWLER_SETTING']['MYSHIPS']['SHIP_DETAIL'], 'headers':self.headers, 'json':input_json, 'timeout':180}, req_retry_limit=3, req_retry_sleeptime=5)
            rsp.close()
        except:
            self.err_count+=1
